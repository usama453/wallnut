/* Smoke test for the zero-cost pipeline: image -> normalize -> OCR -> mock AI report. */
import sharp from "sharp";
import { normalizeImage } from "../src/lib/image.ts";
import { extractText } from "../src/lib/ocr/tesseract.ts";
import { MockProvider } from "../src/lib/ai/mock.ts";
...
async function main() {
  ...
  if (report.issues.length === 0) {
    console.error("FAIL: expected mock provider to find issues in this artwork");
    process.exit(1);
  }
